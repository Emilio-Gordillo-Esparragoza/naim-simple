...
class TabularMasker:
    ...
    def mask(self, data: Tensor):
        ...
        masks = missing.unsqueeze(1).expand(-1, feature_count, -1)

        if self.mask_type == 1:
            present = ~masks
            masks = ~(present & present.transpose(-2, -1))

        additive_mask = torch.zeros_like(masks, dtype=data.dtype)
        additive_mask = additive_mask.masked_fill(masks, self.missing_value)

        if self.mask_type != 2:
            return additive_mask, None

        return additive_mask, additive_mask.transpose(-2, -1)
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        ), f"`input_size`({input_size}) should be divisible by `num_heads`({num_heads})"
...
class NAIM(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(input_size):
            ...
            feature_type_params = {
                True: dict(cardinalities=[self.cat_dims[j] + 1], padding_idx=self.cat_dims[j]),
                False: dict(cardinalities=[2], padding_idx=1),
            }

            j = j + (is_categorical_feature * (i != self.cat_idxs[-1]))
            embedding = CategoricalFeatureTokenizer(
                **common_params, **feature_type_params[is_categorical_feature]
            )
        ...
        if not self.extractor:
            if self.output_size > 1:
                self.classifier = torch.nn.Sequential(
                    torch.nn.Linear(self.input_size * self.d_token, self.output_size)
                )
            else:
                self.classifier = torch.nn.Sequential(
                    torch.nn.Linear(self.input_size * self.d_token, self.output_size),
    ...
    def forward(self, x):
        ...
        for feature_idx in list(range(x.shape[1])):
            if feature_idx in self.cat_idxs:
                single_feature = torch.nan_to_num(x[:, feature_idx], nan=self.cat_dims[j]).to(
                    torch.int64
                )
                feature_values = None
                j += 1
            ...
            feature_embeddings.append(single_feature_embedding)
